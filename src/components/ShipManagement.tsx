...
import { Ship, Equipment, ShipClass, EquipmentSlot } from '../types/game';
import { shipTemplates, createShip } from '../data/ships';
...
export const ShipManagement: React.FC = () => {
  ...
  const handleBuyShip = (shipClass: ShipClass) => {
  ...
  return (
    <div className="bg-gray-800 rounded-lg p-6 mb-6">
      ...
      {/* Ship Purchase */}
      <div className="mb-6">
        <h3 className="text-xl font-bold mb-4">Available Ships</h3>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          {(Object.entries(shipTemplates) as [ShipClass, Ship][]).map(([className, template]) => (
            <div key={className} className="bg-gray-700 p-4 rounded-lg">
              <div className="flex justify-between items-center mb-2">
                <span className="capitalize font-semibold">{className}</span>
                <span>{template.cost} credits</span>
              </div>
              <div className="flex gap-4 mb-4">
                {(Object.entries(template.maxEquipmentSlots) as [EquipmentSlot, number][]).map(([slot, max]) => (
                  <div key={slot} className="flex items-center gap-1">
                    {slotIcons[slot]}
                    <span>{max}</span>
                  </div>
              ...
              <button
                onClick={() => handleBuyShip(className)}
